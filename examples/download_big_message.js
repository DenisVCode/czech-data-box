...
async function downloadBigMessage(messageId) {
  ...
  try {
    const message = await isdsBox.downloadBigMessage(messageId);
...
async function main() {
  if (process.argv.length < 3) {
    console.error('Usage: node examples/download_big_message.js <messageId>');
    process.exit(1);
  }
  ...
  try {
    await downloadBigMessage(messageId);
  } catch (error) {
